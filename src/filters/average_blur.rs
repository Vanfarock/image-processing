...
use crate::blur::kernel::get_average_kernel;
use crate::custom_image::custom_image::{CustomImage, ImageFilter};
use crate::pixel_algorithms::convolution::convolute;

pub fn run(base_img: &mut DynamicImage, kernel_size: (u32, u32)) -> Box<dyn ImageFilter> {
    let img = &mut base_img.clone();
    let (width, height) = img.dimensions();
    let kernel = get_average_kernel(kernel_size);
